...
class IhsanConstitution:
    ...
    def compute_ihsan_score(self) -> float:
        ...
        This MUST call the canonical Rust implementation via FFI.
        Python implementations violate constitutional determinism.
        """
        try:
            # Import Rust FFI (compiled via maturin)
            from bizra_ffi import compute_ihsan

            # Map to canonical 8-dimensional Ihsan vector
            ihsan_vector = {
                'correctness': self.adl,
                'safety': self.amanah,
                'user_benefit': self.ihsan,
                'efficiency': self.hikmah,
                'auditability': self.sidq,
                'anti_centralization': self.tawadu,
                'robustness': self.sabr,
                'adl_fairness': self.shukr,
            }

            # Call canonical Rust implementation
            return compute_ihsan(ihsan_vector)
        except ImportError:
            raise RuntimeError(
                "FATAL: bizra_ffi module not available. "
