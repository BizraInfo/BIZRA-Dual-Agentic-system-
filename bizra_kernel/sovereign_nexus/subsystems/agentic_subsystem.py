...
class AgenticSubsystem:
    ...
    async def _get_available_agent(self, agent_type: str):
        ...
        # If no suitable agent found, use any agent or create a new one
        if suitable_agent is None:
            if self.warm_pool:
                # Use any available agent from the pool
                suitable_agent = self.warm_pool.pop()
            else:
                # Create a new agent if the pool is empty
                try:
                    suitable_agent = await self.agent_factory.create_agent(agent_type=agent_type)
                except Exception:
                    # If we can't create the specific type, create a general one
                    suitable_agent = await self.agent_factory.create_agent(agent_type="general")
