...
class KernelConfig:
    ...
    enable_envelope_enforcement: bool = True  # New flag
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "enable_envelope_enforcement": self.enable_envelope_enforcement,
        }
...
class SystemProtocolKernel:
    ...
    def __init__(self, config: Optional[KernelConfig] = None):
        ...
        self.replay_guard = ReplayGuard()  # Initialize ReplayGuard
    ...
    def execute(
        ...
        envelope: Optional[Envelope] = None,  # New argument
    ) -> ExecutionResult:
        """
        Execute a kernel-managed agent interaction.
        
        This is the main entry point that:
        1. Enforces Evidence Envelope (if enabled)
        2. Creates a protocol-hashed session
        3. Runs verification
        4. Computes Ihsān score
        5. Tracks SNR
        6. Checks for SAPE elevation
        7. Returns complete execution result
        """
        # 0. Enforce Envelope
        if self.config.enable_envelope_enforcement:
            if not envelope:
                raise ValueError("KERNEL REJECTION: Evidence Envelope required but missing.")
            
            # Helper to validate envelope against kernel's policy hash
            # For strict Node-0 enforcement, we require exact match.
            if envelope.policy_hash != self.protocol_hash:
                raise ValueError(
                    f"KERNEL REJECTION: Policy hash mismatch. "
                    f"Expected {self.protocol_hash[:8]}..., got {envelope.policy_hash[:8]}..."
                )

            try:
                self.replay_guard.validate_envelope(envelope)
            except ValueError as e:
                raise ValueError(f"KERNEL REJECTION: Envelope validation failed - {str(e)}")
