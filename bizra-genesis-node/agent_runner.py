...
def create_envelope(
    ...
    """Create a signed Evidence Envelope for the request."""
    ...
    # Simple payload hash (agent + message)
    payload_str = f"{agent_role}:{message}"
...
def run_pat_agent(
    ...
    enforce_kernel: bool = True,
    envelope: Optional["Envelope"] = None  # New arg
) -> dict:
    """
    ...
    if KERNEL_AVAILABLE and enforce_kernel and not response.startswith("ERROR"):
        try:
            ...
            execution = kernel.execute(
                agent=role.value,  # Fix: was action_type/agent_id/payload mismatch in previous code vs current kernel.py
                query=message,
                response=response,
                knowledge_context=knowledge_context,
                user_id=envelope.session_id if envelope else "anonymous", # Bind to session
                envelope=envelope
            )
...
def run_sat_agent(
    ...
    **kwargs
    ...
    """
    Run a SAT agent (rule-based, no LLM) with Kernel/Envelope enforcement.
    
    SAT agents produce system-level outputs that are still governed by
    the Evidence Envelope protocol for auditability and replay protection.
    """
    config = SAT_AGENTS[role]
    
    # Kernel enforcement for SAT outputs
    if KERNEL_AVAILABLE and enforce_kernel:
        kernel = get_kernel()
        
        if kernel.config.enable_envelope_enforcement:
            if not envelope:
                return {
                    "agent": role.value,
                    "error": "KERNEL REJECTION: Evidence Envelope required for SAT execution.",
                    "success": False,
                }
            
            try:
                kernel.replay_guard.validate_envelope(envelope)
            except ValueError as e:
                return {
                    "agent": role.value,
                    "error": f"KERNEL REJECTION: {str(e)}",
                    "success": False,
                }

    # Execute the SAT logic
    result = {"agent": role.value, "success": True}
    ...
    if role == SatRole.PoiVerifier:
        ...
        result.update({"status": "not implemented"})
    
    # Attach envelope hash if present (for audit trail)
    if envelope:
        result["envelope_hash"] = envelope.compute_envelope_hash()[:16]
...
def interactive_mode():
    ...
    # Get initial policy hash from kernel if available
    if KERNEL_AVAILABLE:
        try:
            policy_hash = get_kernel().protocol_hash
            print(f"  [Info] Bound to Policy Hash: {policy_hash[:8]}...")
        except Exception:
            pass
    ...
    while True:
        ...
        if KERNEL_AVAILABLE:
            ...
            envelope = create_envelope(
                agent_role=current_agent.value,
                message=user_input,
                session_id=session_id,
...
def _env_is_production() -> bool:
    # Treat anything explicitly marked prod/staging as production-grade.
    env = (os.getenv("BIZRA_ENV") or os.getenv("NODE_ENV") or "").lower().strip()
...
def parse_args():
    parser = argparse.ArgumentParser(description="BIZRA Agent Runner")
    ...
    parser.add_argument(
        "--no-kernel",
        action="store_true",
        help="Disable protocol kernel enforcement (DEV ONLY; forbidden in prod/staging)",
    )
...
async def main():
    ...
    # 1) Forbid disabling kernel in production-like environments
    if args.no_kernel and _env_is_production():
        _fatal("Kernel enforcement cannot be disabled in prod/staging. Remove --no-kernel or change BIZRA_ENV.")

    # 2) Fail-closed if kernel is required but unavailable
    if (not args.no_kernel) and (not KERNEL_AVAILABLE):
        _fatal(
            "Protocol Kernel is REQUIRED but not available. Install bizra_system_protocol "
            "or run explicitly with --no-kernel (DEV ONLY)."
        )
    ...
    if args.sat:
        try:
            ...
            # Generate envelope for SAT execution
            sat_envelope = None
            if KERNEL_AVAILABLE and (not args.no_kernel):
                sat_session_id = str(uuid.uuid4())
                try:
                    sat_policy_hash = get_kernel().protocol_hash
                except Exception:
                    sat_policy_hash = ""
                    
                sat_envelope = create_envelope(
                    agent_role=role.value,
                    message=f"SAT:{role.value}",
                    session_id=sat_session_id,
                    counter=1,
                    policy_hash=sat_policy_hash
                )
            
            result = run_sat_agent(
                role, 
                envelope=sat_envelope, 
                enforce_kernel=not args.no_kernel
            )
    ...
    if args.agent and args.message:
        ...
        # Prepare envelope for one-shot execution
        os_envelope = None
        if KERNEL_AVAILABLE and (not args.no_kernel):
             os_session_id = str(uuid.uuid4())
             try:
                 os_policy_hash = get_kernel().protocol_hash
             except Exception:
                 os_policy_hash = ""
                 
             os_envelope = create_envelope(
                 agent_role=role.value,
                 message=args.message,
                 session_id=os_session_id,
                 counter=1,
                 policy_hash=os_policy_hash
             )
