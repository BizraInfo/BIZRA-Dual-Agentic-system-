...
-- pgvector for embeddings (Generic fallback: FLOAT8[])
-- ============================================
...
CREATE TABLE IF NOT EXISTS user_profile (
    ...
    seed_state TEXT NOT NULL CHECK (
        seed_state IN (
            'dreamer',
            'builder',
            'learner',
    ...
    primary_pat_role TEXT NOT NULL CHECK (
        primary_pat_role IN (
            'MasterReasoner',
            'MemoryArchitect',
            'CreativeSynthesizer',
...
CREATE TABLE IF NOT EXISTS poi_ledger (
    ...
    event_type TEXT NOT NULL CHECK (
        event_type IN (
            'task_completed',
            -- User finished work task with PAT
            'resource_contributed',
            -- Node provided compute
            'knowledge_shared',
            -- User added to knowledge base
            'learning_session',
            -- User completed educational module
            'bug_fixed',
            -- User fixed code issue
            'documentation_written',
            -- User created/improved docs
            'onboarding_completed',
            -- User completed onboarding
            'plan_created',
            -- User created 7-day plan
            'daily_checkin',
    ...
    ihsan_score NUMERIC(5, 4) NOT NULL CHECK (
        ihsan_score >= 0
        AND ihsan_score <= 1
    ),
    -- Resources used
    ...
    reward_bzc NUMERIC(20, 8) DEFAULT 0,
    -- BIZRA Coin (utility)
    reward_imp NUMERIC(20, 8) DEFAULT 0,
    -- Impact Token (soulbound)
    -- Timestamps
    created_at TIMESTAMPTZ DEFAULT NOW()
);
-- ============================================
...
CREATE TABLE IF NOT EXISTS resource_pool (
    ...
    status TEXT NOT NULL DEFAULT 'active' CHECK (
        status IN ('active', 'paused', 'offline', 'maintenance')
    ),
    -- Statistics
...
CREATE TABLE IF NOT EXISTS system_health (
    ...
    -- Overall status
    overall_status TEXT DEFAULT 'healthy' CHECK (
        overall_status IN ('healthy', 'degraded', 'critical', 'offline')
    )
);
-- ============================================
...
UPDATE ON user_profile FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_asset_registry_updated_at BEFORE
UPDATE ON asset_registry FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_resource_pool_updated_at BEFORE
UPDATE ON resource_pool FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_pat_sessions_updated_at BEFORE
UPDATE ON pat_sessions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE OR REPLACE FUNCTION calculate_poi_rewards(
    ...
    ) RETURNS TABLE(bzc_reward NUMERIC, imp_reward NUMERIC) AS $$ BEGIN -- BZC (utility token) = impact * duration * 0.1
...
DECLARE v_ihsan NUMERIC;
v_impact NUMERIC;
v_result BOOLEAN := FALSE;
BEGIN -- Get event scores
SELECT ihsan_score,
    impact_score INTO v_ihsan,
    v_impact
FROM poi_ledger
WHERE id = p_event_id;
-- Check Ihsan threshold
IF v_ihsan >= p_ihsan_threshold THEN -- Mark as verified
UPDATE poi_ledger
SET verified = TRUE,
    verified_at = NOW(),
...
-- ============================================
CREATE OR REPLACE FUNCTION get_user_poi_stats(p_user_id TEXT DEFAULT 'NODE0-USER') RETURNS TABLE(
        total_events BIGINT,
...
VALUES (
        'NODE0-TITAN',
        24,
        -- i9-14900HX has 24 cores (32 threads)
        4,
        -- Default allocation: 4 cores
        TRUE,
        -- GPU enabled
        16,
...
VALUES (
        ...
        100.0,
        1.0,
        'BIZRA Genesis Node Constellation Activated',
        TRUE,
        NOW()
    ),
    (
        ...
        50.0,
        0.99,
        'Initial SAPE Validator Logic Anchored',
        TRUE,
        NOW()
    ),
    (
