...
impl AssetRegistry {
    ...
    pub async fn search(&self, query: &str, limit: i64) -> anyhow::Result<Vec<Asset>> {
    ...
    pub async fn get_stats(&self) -> anyhow::Result<AssetStats> {
        ...
        for row in domain_counts {
            by_domain.insert(row.get("domain"), row.get("count"));
        }
        ...
        for row in type_counts {
            by_type.insert(row.get("file_type"), row.get("count"));
        }

        Ok(AssetStats {
            total_assets: basic_stats.get("total_assets"),
            indexed_assets: basic_stats.get("indexed_assets"),
            total_size_bytes: basic_stats.get("total_size_bytes"),
            by_domain,
            by_type,
        })
    }
