...
impl PoiLedger {
    ...
    pub async fn log_event(
        ...
        // Calculate rewards
        let (bzc_reward, imp_reward) =
            Self::calculate_rewards(impact_score, ihsan_score, duration_minutes.unwrap_or(1));

        let event = sqlx::query_as::<_, PoiEvent>(
            r#"
    ...
    pub async fn verify_event(&self, event_id: Uuid) -> anyhow::Result<bool> {
        ...
        if event.get::<f64, _>("ihsan_score") >= self.ihsan_threshold {
            sqlx::query(
                r#"
                UPDATE poi_ledger 
    ...
    pub async fn get_stats(&self, user_id: &str) -> anyhow::Result<PoiStats> {
        ...
        Ok(PoiStats {
            ...
            verified_events: stats.get("verified_events"),
            total_impact: stats.get("total_impact"),
            avg_ihsan: stats.get("avg_ihsan"),
            total_minutes: stats.get("total_minutes"),
            total_bzc: stats.get("total_bzc"),
            total_imp: stats.get("total_imp"),
        })
    }
    ...
    pub async fn get_timeline(
        ...
        let events = sqlx::query_as::<_, PoiEvent>(
            r#"
            SELECT 
    ...
    pub async fn batch_verify(&self) -> anyhow::Result<(i64, i64)> {
        ...
        for row in unverified {
            ...
            if self.verify_event(id).await.unwrap_or(false) {
                verified_count += 1;
            }
        }
...
mod tests {
    ...
    fn test_poi_event_type_display() {
        ...
        assert_eq!(
            PoiEventType::LearningSession.to_string(),
            "learning_session"
        );
    }
}
