...
use crate::services::knowledge::knowledge_client;
...
impl PatRole {
    ...
    pub fn model(&self) -> &'static str {
        match self {
            ...
            Self::MemoryArchitect | Self::CreativeSynthesizer | Self::EthicsGuardian => {
                "qwen2.5:7b"
            }
...
Consider task dependencies and optimal sequencing."#,
        ...
        let plan: serde_json::Value =
            serde_json::from_str(&response.response).unwrap_or_else(|_| {
...
mod tests {
    ...
    fn test_pat_role_from_str() {
        assert_eq!(
            PatRole::from_str("MasterReasoner"),
            Some(PatRole::MasterReasoner)
        );
