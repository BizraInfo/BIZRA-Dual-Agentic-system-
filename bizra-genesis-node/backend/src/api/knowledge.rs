...
use crate::services::knowledge::HypergraphClient;
use axum::{
    extract::{Path, State},
    http::StatusCode,
    response::Json,
...
async fn query_knowledge(
    ...
    let result = state.client.query(&request.query).await.map_err(|e| {
        (
...
async fn find_by_concept(
    ...
    let sources = state.client.find_by_concept(&concept).await.map_err(|e| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(ErrorResponse {
                error: e.to_string(),
...
async fn knowledge_status(State(state): State<Arc<KnowledgeState>>) -> Json<StatusResponse> {
