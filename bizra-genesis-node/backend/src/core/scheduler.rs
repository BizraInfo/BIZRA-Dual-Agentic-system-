...
impl TaskScheduler {
            ...
            match result {
                Ok(result_value) => {
                    ...
                    metrics
                        ...
                        .fetch_add(elapsed.as_millis() as u64, AtomicOrdering::Relaxed);
                }
                Err(error) => {
    ...
    fn unblock_dependents(state: &mut SchedulerState, completed_id: TaskId) {
        let to_unblock: Vec<TaskId> = state
            .blocked_tasks
            .iter()
            .filter(|&tid| {
                state
...
mod tests {
    ...
    async fn test_task_dependencies() {
        ...
        let child = Task::new("child", serde_json::json!({})).depends_on(parent_id);
    ...
    async fn test_circular_dependency_detection() {
        ...
        let task_b = Task::new("B", serde_json::json!({})).depends_on(task_a_id);
        let task_b_id = task_b.id;
        scheduler.submit(task_b).await.unwrap();

        // This should fail due to circular dependency
        let mut task_c = Task::new("C", serde_json::json!({})).depends_on(task_b_id);
    ...
    async fn test_priority_ordering() {
        ...
        let low = Task::new("low", serde_json::json!({})).with_priority(TaskPriority::Low);
        scheduler.submit(low).await.unwrap();

        // Submit high priority second
        let high = Task::new("high", serde_json::json!({})).with_priority(TaskPriority::High);
