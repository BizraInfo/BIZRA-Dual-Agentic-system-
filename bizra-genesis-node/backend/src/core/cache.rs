    ...
    V: Clone + Send + Sync + Serialize + DeserializeOwned + 'static,
{
    ...
    pub fn new(config: MultiTierConfig) -> Self {
        let l1 = Arc::new(LruCache::new(config.l1_capacity).with_default_ttl(config.l1_ttl));
...
impl BizraCaches {
    ...
    pub fn new() -> Self {
        Self {
            profiles: Arc::new(LruCache::new(1000).with_default_ttl(Duration::from_secs(300))),
            responses: Arc::new(LruCache::new(5000).with_default_ttl(Duration::from_secs(3600))),
            rate_limits: Arc::new(LruCache::new(10000).with_default_ttl(Duration::from_secs(60))),
            sessions: Arc::new(LruCache::new(5000).with_default_ttl(Duration::from_secs(1800))),
        }
    }
...
mod tests {
    ...
    async fn test_ttl_expiration() {
        let cache: LruCache<String, i32> =
            LruCache::new(10).with_default_ttl(Duration::from_millis(50));
    ...
    async fn test_bizra_caches() {
        ...
        caches
            .profiles
            .put("user1".to_string(), serde_json::json!({"name": "Test"}))
            .await;
