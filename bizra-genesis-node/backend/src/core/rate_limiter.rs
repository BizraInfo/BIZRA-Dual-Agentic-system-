...
impl<K: Eq + Hash + Clone + Send + Sync> TokenBucketLimiter<K> {
    ...
    pub async fn check(&self, key: &K) -> RateLimitDecision {
        self.try_acquire(key, self.config.tokens_per_request, false)
            .await
    }
    ...
    pub async fn acquire(&self, key: &K) -> Result<RateLimitDecision, RateLimitError> {
        let decision = self
            .try_acquire(key, self.config.tokens_per_request, true)
            .await;
        if decision.allowed {
            Ok(decision)
        } else {
    ...
    async fn try_acquire(&self, key: &K, tokens_needed: u64, consume: bool) -> RateLimitDecision {
        ...
        let bucket = buckets.entry(key.clone()).or_insert_with(|| {
            let initial = self
                .config
                .initial_tokens
                .unwrap_or(self.config.bucket_capacity)
                * PRECISION;
            TokenBucket {
                tokens: initial,
                last_refill: now,
        ...
        let refill_amount =
            (elapsed.as_secs_f64() * self.config.refill_rate * PRECISION as f64) as u64;
        ...
        if bucket.tokens >= tokens_needed_scaled {
            ...
            let retry_after = (tokens_deficit as f64 / (self.config.refill_rate * PRECISION as f64)
                * 1000.0) as u64;
            let reset_after = self.calculate_reset_time(bucket.tokens, capacity_scaled);
    ...
    pub async fn acquire(&self, key: &K) -> Result<RateLimitDecision, RateLimitError> {
        ...
        if let Some(limiter) = limiters.get(key) {
            ...
            if let Some(limiter) = limiters.get(key) {
                ...
                Err(RateLimitError::ConfigError(
                    "Failed to create limiter".into(),
                ))
            }
        }
    }
...
impl RateLimiterMetrics {
    ...
    pub fn total_requests(&self) -> u64 {
        self.requests_allowed.load(Ordering::Relaxed) + self.requests_denied.load(Ordering::Relaxed)
    }
...
mod tests {
    ...
    async fn test_quota_manager() {
        let manager: QuotaManager<String> = QuotaManager::new(Quota::per_second(10));

        // Set custom quota for VIP user
        manager
            .set_quota("vip-user".to_string(), Quota::per_second(100))
            .await;
