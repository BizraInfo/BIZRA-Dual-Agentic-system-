    ...
    pub fn validate_receipt(&mut self, receipt: &PoiReceipt) -> Result<()> {
        ...
        if let Some(existing_node) = self.receipt_cache.get(&receipt.output_hash) {
            if existing_node != &receipt.node_uid.to_string() {
                bail!("DUPLICATE_OUTPUT_DETECTED: Hash {} already submitted by node {}", 
                      receipt.output_hash, existing_node);
            }
        }
        ...
        // 3. Commit to cache
        self.receipt_cache.insert(receipt.output_hash.clone(), receipt.node_uid.to_string());
