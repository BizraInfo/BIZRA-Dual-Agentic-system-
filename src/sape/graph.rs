...
impl ReasoningGraph {
    ...
    pub fn add_node(&mut self, content: String, node_type: NodeType, snr: f64, ihsan: f64) -> String {
    ...
    pub fn add_edge(&mut self, from: &str, to: &str, edge_type: EdgeType, weight: f64) {
        if self.nodes.contains_key(from) && self.nodes.contains_key(to) {
            ...
            warn!("Attempted to add edge between non-existent nodes: {} -> {}", from, to);
        }
    }
    ...
    pub fn verify_causality(&self, target_node_id: &str) -> bool {
         ...
         while let Some(current_id) = stack.pop() {
             ...
             if let Some(node) = self.nodes.get(&current_id) {
                 ...
                 if matches!(node.node_type, NodeType::Evidence | NodeType::Axiom | NodeType::Initial) {
                     has_grounding = true;
             ...
             for edge in &self.edges {
                 if edge.to == current_id { // Incoming edge
                    // Only traverse structural/supporting edges for causality
                    if matches!(edge.edge_type, EdgeType::Follows | EdgeType::Supports | EdgeType::CausalLink | EdgeType::Provenance) {
    ...
    pub fn prune(&mut self, min_snr: f64, min_ihsan: f64) -> usize {
        ...
        let nodes_to_keep: HashSet<String> = self.nodes.iter()
            .filter(|(id, node)| {
                // Score check
                ...
                let causality_pass = if matches!(node.node_type, NodeType::Final | NodeType::Convergent) {
        ...
        self.edges.retain(|edge| nodes_to_keep.contains(&edge.from) && nodes_to_keep.contains(&edge.to));
    ...
    pub fn get_best_path(&self) -> Option<Vec<&GraphNode>> {
        ...
        let best_final = self.nodes.values()
            .filter(|n| n.node_type == NodeType::Final)
            .max_by(|a, b| (a.ihsan_score + a.snr_score).partial_cmp(&(b.ihsan_score + b.snr_score)).unwrap_or(std::cmp::Ordering::Equal));
        ...
        if let Some(target) = best_final {
             ...
             loop {
                 let parents: Vec<&GraphEdge> = self.edges.iter().filter(|e| e.to == current.id && e.edge_type == EdgeType::Follows).collect();
                 if parents.is_empty() {
                     break;
                 }
                 // If multiple parents, pick best
                 if let Some(best_parent_edge) = parents.iter().max_by(|a, b| a.weight.partial_cmp(&b.weight).unwrap()) {
...
mod tests {
    ...
    fn test_causality_verification() {
        ...
        let conclusion_id = graph.add_node(
        ...
        assert!(!graph.verify_causality(&conclusion_id), "Disconnected node should fail causality check");

        // Case B: Linked to Thought only (Floating chain) -> Should Fail
        graph.add_edge(&thought_id, &conclusion_id, EdgeType::Follows, 1.0);
        assert!(!graph.verify_causality(&conclusion_id), "Floating chain should fail (no root)");

        // Case C: Linked to Evidence -> Should Pass
        graph.add_edge(&evidence_id, &thought_id, EdgeType::Supports, 1.0);
        assert!(graph.verify_causality(&conclusion_id), "Fully grounded chain should pass");
    }
}
