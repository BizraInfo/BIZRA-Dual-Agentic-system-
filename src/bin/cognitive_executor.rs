...
async fn main() -> anyhow::Result<()> {
    ...
    let module_bytes = match fs::read(wasm_path) {
        ...
        Err(_) => {
            ...
            eprintln!("⚠️ Brain not found at {}. Using Lobotomized Stub.", wasm_path);
    ...
    let (mut agent_result, evidence_chain) = cognitive.execute_thought(&capsule, &input_string).await?;
    ...
    if let Ok(eval_result) = serde_json::from_str::<serde_json::Value>(&agent_result.contribution) {
        if let Some(s) = eval_result.get("score").and_then(|v| v.as_f64()) {
            score = s;
            println!("🧠 Brain Verdict: Score {}, Reason: {:?}", score, eval_result["reasoning"]);
        }
    } else {
        println!("⚠️ Brain returned unintelligible speech: {}", agent_result.contribution);
    }
