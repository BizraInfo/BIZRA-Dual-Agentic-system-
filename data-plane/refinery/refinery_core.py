...
class IhsanRefinery:
    ...
    def calculate_impact(self, path: Path, size_mb: float) -> float:
        ...
        # ELITE WISDOM BOOST (Phase 1 Synthesis Integration)
        path_str = str(path).lower()
        wisdom_multiplier = 1.0
        if "quran" in path_str:
            wisdom_multiplier = 100.0 # Universal Truth Anchor
        elif "hadith" in path_str or "sunnah" in path_str:
            wisdom_multiplier = 50.0  # Prophetic Protocol Anchor
        elif "wisdom" in path_str or "ihsan" in path_str:
            wisdom_multiplier = 25.0  # Core Philosophy Anchor
            
        return round(size_mb * multiplier * wisdom_multiplier, 4)
